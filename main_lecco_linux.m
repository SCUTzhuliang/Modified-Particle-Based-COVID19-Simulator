% prepare the desktop
function [myresult] = main_lecco_linux()
%% initialize parameters
num_simulations = 10;                          % Total number of simulations
num_population = 100000;                       % Total number of particles
...
init_max_vel = 0.040;                         % Maximum allowed velocity 
...
masked_rate = 0.40;
num_masked = masked_rate * num_population;
dist_decrement = 0.80;
day_delay = 1;
ind_delay = (day_delay/delta_t);
mtrace_rate = 0.60;

%save_figures = 0;          % Save 2d map and plot. 0: no, 1: yes.
%vis = num_iter;            % plot figures after each vis iterations
...
tracking_rate = 0.5;          % Percentage of population using tracking app
testing_rate = 10e-3;        % Daily tests per thousand people
...
tot_r0_n = zeros(num_iter, num_simulations);

%x_ini = load('initialization/x.mat');
%v_ini = load('initialization/v.mat');
%ind_exposed_ini = load('initialization/ind_exposed.mat');
starter = tic;
%% loop over the number of simulations
parfor num_sim = 1:num_simulations
    ...
    contactHistory = cell(num_population, 2);
    ...
    ind_masked = zeros(num_population, 1);
    
    % load indices of exposed particles
    if load_init_states == 1
        %ind_exposed = ind_exposed_ini
    % otherwise choose randomly
    else
        ind_exposed = randi([1 num_population], 1 , num_exposed);
        ind_masked = randi([1 num_population], 1 , num_masked);
    end
    e(ind_exposed) = 1;
    mask(ind_masked) = 1;
...
%         end
        ...
        temp_ind_masked = temp_ind & mask;
        temp_ind_unmasked = temp_ind & not(mask);
        
        if mod(ind, kdt) == 0 || ind == 1
            ns = createns(x, 'nsmethod', 'kdtree', 'distance', 'cityblock');
        end
        
        [ind_contacts_masked, ~] = rangesearch(ns, x(temp_ind_masked,:), dist_decrement*contact_thres);
        [ind_contacts_unmasked, ~] = rangesearch(ns, x(temp_ind_unmasked,:), contact_thres);
        
        ind_contacts = [ind_contacts_masked;ind_contacts_unmasked];
        ...
        len = zeros(numel(ind_contacts), 1);
        for i = 1:numel(ind_contacts)
            % index of the current contact
            if isempty(ind_contacts{i})
                continue
            end
            len(i) = length(ind_contacts{i})-1;
            curr_ind = ind_contacts{i}(1);
            % Dead, quarantined, isolated and recovered don't contact
            % e(curr_ind) ~= 3 && e(curr_ind) ~= 4
            % (e(curr_ind) == 0 || e(curr_ind) == 1 || e(curr_ind) == 2 || e(curr_ind) == 7)
            if (e(curr_ind) == 0 || e(curr_ind) == 1 || e(curr_ind) == 2 || e(curr_ind) == 7)
                ...
                if a(curr_ind) == 1
                    for j = 2:numel(ind_contacts{i})
                        ...
                        if (e(cont_ind) == 0 || e(cont_ind) == 1 || e(cont_ind) == 2 || e(cont_ind) == 7)
                ...
                for j = 2:numel(ind_contacts{i})
                    m = numel(contactHistory{curr_ind});
                    cont_ind = ind_contacts{i}(j);
                    if (e(cont_ind) == 0 || e(cont_ind) == 1 || e(cont_ind) == 2 || e(cont_ind) == 7)
                        contactHistory{curr_ind, 1}(m + 1) = cont_ind;        % Particle ID
                        contactHistory{curr_ind, 2}(m + 1) = ind * delta_t;   % Date
                    end
                end
            end
        end
        if(numel(ind_contacts) ~=  0)
            tot_r0(ind) = mean(len);
        end
        ...
        ind_prev_inf = ((ts_t >= (ind-1-ind_delay) & ts_t < (ind-ind_delay)) | (ts_f >= (ind-1-ind_delay) & ts_f < (ind-ind_delay)));
        for i = 1:numel(ind_prev_inf)
            % if the recently infected particle has installed app
            if ind_prev_inf(i) == 1 
                % get the number of contacts of the particle
                m = numel(contactHistory{i,1});
                temp = rand(m,1);
                for j = 1:m
                    % if they were in contact within the last 14 days
                    if (temp(j) < mtrace_rate && contactHistory{i,2}(j) >= (ind * delta_t - t_inf - day_delay))
                        ...
                        if e(contactHistory{i,1}(j)) == 0
                            e(contactHistory{i,1}(j)) = 9;
                            t(contactHistory{i,1}(j)) = 0;
                        % else if it is exposed then quarantine it
                        % without changing the time
                        elseif e(contactHistory{i,1}(j)) == 1
                            e(contactHistory{i,1}(j)) = 5;
                        % else if it is infected then isolate
                        elseif e(contactHistory{i,1}(j)) == 2
                            e(contactHistory{i,1}(j)) = 6;
                        end
                    end
                end
            end
        ...
        if ind == num_iter
            disp(['simulation: ', num2str(num_sim),'finished in sim time (sec): ', num2str(toc)]);
        end
    ...
    tot_r0_n(:, num_sim) = tot_r0;
end

time_record = toc(starter)

myresult.time_record =  time_record;
myresult.num_simulations =  num_simulations;
myresult.num_population =  num_population;
myresult.init_max_vel =  init_max_vel;
myresult.masked_rate =  masked_rate;
myresult.dist_decrement =  dist_decrement;
myresult.day_delay =  day_delay;
myresult.mtrace_rate =  mtrace_rate;
myresult.delta_t =  delta_t;
myresult.num_iter =  num_iter;
myresult.tot_susceptible_n =  tot_susceptible_n;
myresult.tot_exposed_n =  tot_exposed_n;
myresult.tot_infected_n =  tot_infected_n;
myresult.tot_recovered_n =  tot_recovered_n;
myresult.tot_dead_n =  tot_dead_n;
myresult.tot_quarantined_n =  tot_quarantined_n;
myresult.tot_isolated_n =  tot_isolated_n;
myresult.tot_severe_inf_n =  tot_severe_inf_n;
myresult.tot_cases_n =  tot_cases_n;
myresult.tot_r0_n =  tot_r0_n;
