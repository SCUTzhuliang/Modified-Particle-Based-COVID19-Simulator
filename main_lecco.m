...
num_population = 50000;                       % Total number of particles
...
init_max_vel = 0.030;                          % Maximum allowed velocity 
...
manual_tracing = 0;
% day_delay = 2;
% ind_delay = ceil(day_delay/delta_t);
% mtrace_rate = 1.00;
...
tracking_rate = 0.10;          % Percentage of population using tracking app
...
% vis = 2000;            % plot figures after each vis iterations
...
manual_trace_list = cell(num_simulations,1);
digital_trace_list= cell(num_simulations,1);
manual_trace_number = zeros(num_simulations,1);
digital_trace_number = zeros(num_simulations,1);
...
parfor num_sim = 1:num_simulations
    ...
    if load_init_states == 1
        %x = x_ini
        %v = v_ini
        warning('Unexpected init states');
    ...
    mask = rand(num_population, 1) < masked_rate;
    ...
    contactHistory = cell(num_population, 2);          % For manual tracing
    countnum_m = 0;
    countnum_d = 0;
...
%         end
                ...
                for i = 1:numel(ind_prev_inf)
                    if ind_prev_inf(i) == 1 
                        ...
                        for j = 1:m
                            ...
                            if (temp(j) < mtrace_rate && contactHistory{i,2}(j) >= (ind * delta_t - t_inf - day_delay) && contactHistory{i,2}(j) <= (ind * delta_t - day_delay))
        ...
        if ind == num_iter
            manual_trace_list{num_sim} = contactHistory;
            digital_trace_list{num_sim} = contactCell;
            manual_trace_number(num_sim) = countnum_m;
            digital_trace_number(num_sim) = countnum_d;
        end
...
save 50_0.030_dt_2_0.10.mat
disp('50_0.030_dt_2_0.10')